...
app.post("/login", async (req, res) => {
  ...
  try {
    const query = requestedRole
      ? "SELECT user_id, email, password, role, tenant_id FROM users WHERE email = ? AND role = ? LIMIT 1"
      : "SELECT user_id, email, password, role, tenant_id FROM users WHERE email = ? LIMIT 1";

    const params = requestedRole ? [email, requestedRole] : [email];
    const [users] = await db.execute(query, params);

    if (users.length === 0) {
      return sendError(res, 401, "Invalid email/role or password");
    }
